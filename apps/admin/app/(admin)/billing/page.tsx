...
export default function Page() {
  ...
  const revoke = async (profileId: string) => {
    const reason = window.prompt('Reason for revoking this active package?');
    if (!reason?.trim()) return;
    setError(null);
    try {
      await adminApi.revokeBilling(profileId, { reason });
      await load();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Failed to revoke package.';
      setError(message);
    }
  };
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.profile_id} className="border-t border-soft">
                ...
                <td className="py-3">
                  <div className="mb-2 text-xs uppercase tracking-[0.18em] text-white/45">{item.status.replace(/_/g, ' ')}</div>
