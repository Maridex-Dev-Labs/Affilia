...
export default function AdminSidebar() {
  ...
  return (
    <aside className="custom-scrollbar hidden h-screen w-[290px] shrink-0 overflow-y-auto border-r border-white/8 bg-[#141A2B]/84 pt-6 lg:sticky lg:top-0 lg:flex lg:flex-col">
      <div className="px-4 pb-6">
        <div className="rounded-[1.6rem] border border-[#BB0000]/20 bg-black/40 p-5">
          ...
          <div className="mt-2 text-2xl font-black italic text-white">Affilia Control</div>
        ...
        {visibleNav.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`relative flex items-center gap-4 rounded-xl p-3 text-sm font-bold transition-colors ${active ? 'bg-black/45 text-white' : 'text-[#8f98ab] hover:bg-white/5 hover:text-white'}`}
            >
